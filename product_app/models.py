...
class Costumer(models.Model):
    ...
    costumer_last_name = models.CharField(max_length=300)
    phone_number = models.CharField(max_length=11, unique=True)
    date_created = models.DateTimeField(default=datetime.now())
    address = models.CharField(max_length=300)

    def __str__(self):
        return '{} {}'.format(self.costumer_first_name, self.costumer_last_name)

    def get_details(self):
        details = {"full_name": "{} {}".format(self.costumer_first_name.capitalize(), self.costumer_last_name.capitalize()),
    ...
    def get_transaction_history(self):
        details = {
            "all_orders": Order.objects.filter(costumer=self).order_by("date_created"),
            'outstanding_orders' : Order.objects.filter(costumer=self, order_completed = True, paid_fully =False).order_by("date_created"),
...
class SubOrder(models.Model):
    ...
    def __str__(self):
        return '{} {} at NGN {}'.format(self.product_ordered.sub_type_name, self.quantity, self.buy_price)
...
def get_orders_based_on_date(start_date, end_date):
    orders = Order.objects.filter(date_created__range=[start_date, end_date +  timedelta(days=1)]).filter(order_completed=True)
